...
public class VideoAdapter extends RecyclerView.Adapter<VideoAdapter.MediaViewHolder>{
    ...
    public void onBindViewHolder(MediaViewHolder viewHolder, int i) {
        ActivityMediaVideo ac = new ActivityMediaVideo();
        viewHolder.nombre.setText(items.get(i).getId_media()+" - "+items.get(i).getId_TareaNota()+" - "+items.get(i).getDescripMedia());
        viewHolder.ruta.setText("Ruta: "+items.get(i).getDescripMedia());
        viewHolder.video.setVideoPath("/sdcard/Movies/GuardaVideo/"+items.get(i).getDescripMedia());
    ...
    /**
     * GuardaVideo_20211128_194245.mp4
     * Escala un Bitmap
     *
     * @param uri
     * @param factor
